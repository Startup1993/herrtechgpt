...
export default function AgentLanding({ agent }: { agent: AgentDefinition }) {
  ...
  const isGuided = agent.mode === 'guided'
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 flex flex-col items-center justify-center px-6">
        ...
        <p className="text-sm text-muted max-w-md text-center mb-8">{agent.description}</p>

        {isGuided ? (
          <button
            onClick={() => handleSend("Los geht's! Bitte starte den Prozess.")}
            disabled={loading}
            className="px-6 py-3 bg-primary hover:bg-primary-hover disabled:opacity-60 text-white font-medium rounded-xl transition-colors text-sm"
          >
