...
type Step = 'upload' | 'transcribing' | 'analyzing' | 'done'
...
function StepIndicator({ current }: { current: Step }) {
  const steps: { id: Step | 'transcribing' | 'analyzing'; label: string }[] = [
    { id: 'upload', label: 'Upload' },
    { id: 'transcribing', label: 'Transkription' },
    { id: 'analyzing', label: 'KI-Analyse' },
    { id: 'done', label: 'Ergebnis' },
  ]
  const order = ['upload', 'transcribing', 'analyzing', 'done']
...
export default function VideoEditorWorkflow() {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      const res2 = await fetch('/api/workflows/analyze', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ transcript: data.text, context }),
      })
      const data2 = await res2.json()
      if (!res2.ok) throw new Error(data2.error || 'Analyse fehlgeschlagen')
    ...
    <div className="p-8 max-w-4xl">
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p className="text-xs text-muted">Lade ein Video hoch — Whisper transkribiert, Claude analysiert und empfiehlt Schnitte.</p>
        </div>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-surface border border-border rounded-xl overflow-hidden">
              <div className="px-5 py-3 border-b border-border flex items-center justify-between">
                <h2 className="text-sm font-semibold text-foreground">Transkript mit Timestamps</h2>
                <button
                ...
                {segments.length > 0 ? (
                  segments.map((seg, i) => (
                    <div key={i} className="flex gap-3 text-xs">
                      <span className="text-primary font-mono shrink-0 w-16">
