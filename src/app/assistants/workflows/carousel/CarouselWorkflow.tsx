        ...
        <div className="w-72 shrink-0 flex flex-col gap-3">
          <div className="border border-border rounded-xl bg-surface overflow-hidden">
            <div className="px-4 pt-3 pb-2 border-b border-border">
              <p className="text-xs font-semibold text-foreground">✏️ Per Prompt anpassen</p>
              <p className="text-xs text-muted mt-0.5">Beschreibe was geändert werden soll</p>
            </div>
            <div className="p-3 flex flex-col gap-2">
              <textarea
                ...
                rows={5}
                disabled={refining}
                className="w-full border border-border rounded-lg px-3 py-2.5 text-sm text-foreground bg-surface-secondary focus:outline-none focus:ring-2 focus:ring-primary/30 resize-none placeholder:text-muted disabled:opacity-50"
              />
              <button
                onClick={refineSlides}
                disabled={refining || !refineInput.trim()}
                className="w-full py-2 rounded-lg bg-primary text-white text-sm font-medium flex items-center justify-center gap-2 hover:opacity-90 transition disabled:opacity-40"
              >
                {refining
                  ? <><svg className="animate-spin w-3.5 h-3.5" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg> Wird angepasst...</>
                  : <><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg> Anpassen</>
                }
              </button>
            </div>
          </div>
          ...
          <div className="border border-border rounded-xl bg-surface p-3">
            <p className="text-xs font-semibold text-foreground mb-2">Slides</p>
            <div className="grid grid-cols-4 gap-1.5">
              {slides.map((_, i) => (
                <button key={i} onClick={() => setActiveSlide(i)}
                  className={`h-10 rounded-lg border-2 text-xs font-semibold transition ${i === activeSlide ? 'border-primary text-primary bg-primary/5' : 'border-border text-muted hover:border-primary/40'}`}
                >{i + 1}</button>
              ))}
            </div>
          </div>
